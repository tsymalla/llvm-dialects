...
void VisitorTemplate::add(VisitorKey key, VisitorCallback *fn,
  ...
  if (key.m_kind == VisitorKey::Kind::OpSet)
    handler.opSet = key.m_set;
  ...
  if (key.m_kind == VisitorKey::Kind::Intrinsic) {
    ...
    case OpDescription::Kind::Core:
      for (unsigned op : description->getOpcodes())
    ...
    case OpDescription::Kind::Intrinsic:
      ...
      break;
    default: {
      if (m_opMap.contains(*description)) {
        m_opMap[*description].push_back(handlerIdx);
      } else {
        SmallVector<unsigned> handlers;
        handlers.push_back(handlerIdx);
        m_opMap[*description] = std::move(handlers);
      }
    ...
    for (const auto &dialectOpPair : opSet->getDialectOps()) {
      const OpDescription desc{dialectOpPair.isOverload,
                               dialectOpPair.mnemonic};

      if (m_opMap.contains(desc)) {
        m_opMap[desc].push_back(handlerIdx);
      } else {
        SmallVector<unsigned> handlers;
        handlers.push_back(handlerIdx);
        m_opMap[desc] = std::move(handlers);
      }
    }
  }
}
